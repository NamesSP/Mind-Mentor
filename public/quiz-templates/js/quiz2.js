...
submit.addEventListener('click',()=>{
   ...
   if(checkedAnswer==1){
        ...
        console.log(val)
        if(val>=0 && val<=4){
            submit.innerHTML=`<h3>Severity level: No Anxiety</h3>`
        }
        else if(val>5 && val<=9){
            submit.innerHTML=`<h3>Severity level: Mild Anxiety</h3>`
        }
        else if(val>10 && val<=14){
            submit.innerHTML=`<h3>Severity level: Moderate Anxiety</h3>`
        }
        else if(val>15 && val<=19){
            submit.innerHTML=`<h3>Severity level: Moderately Anxiety</h3>`
        }
        else{
            submit.innerHTML=`<h3>Severity level: Severe Anxiety</h3>`;
        }
        showProblem.innerHTML=`<button class="btn" onclick="location.reload()">Give Test Again </button> <br><button class="btn" onclick="window.location.href='./solution2.html'">See Solutions</button> <br>`
